...
// CustomPasswordField component declaration with destructured props
export const CustomPasswordField = ({ label, name, validation, register, errors, placeholder, disable, col, mandatory, className, style, labelClass, errorColor }) => {
      ...
      <div className={`flex flex-col  ${label ? "gap-2" : ""} relative`}>
        ...
        <div className="relative flex justify-center items-center">
          <input
            ...
            } ${className}`} // Conditional styling based on error state
            placeholder={placeholder}
            {...register(name, validation)} // Register input for validation with React Hook Form
            disabled={disable} // Conditionally disable the input field
            readOnly={disable} // Make input read-only if disabled
          />

          <div onClick={() => setShowPassword(!showPassword)} className="w-fit absolute cursor-pointer right-4 animation ">
            {showPassword ? (
              /*Icon for "show password" state*/
        ...
        {/*Display validation errors if any*/
          errors && <div className={`error ${errorColor === 'white' ? 'text-white': 'text-red-600'}  text-xs`}>{errors?.[name]?.message}</div>
        }
      </div>
    </div>
  );
